...
// --- NEW: Import invoices, payments, and procedures routers ---
...
// --- LOGIN ENDPOINT (UPDATED to include clinic_name) ---
app.post('/api/login', async (req, res) => {
  const { username, password } = req.body;
  try {
    // Fetch user by username, include clinic name via LEFT JOIN
    ...
    // For demo: check plain text password. Replace with bcrypt in production.
...
// --- PATIENTS CRUD with clinic-based filtering ---
app.get('/patients', async (req, res) => {
  // Accept clinic_id from query params for frontend compatibility
  const clinic_id = req.query.clinic_id;
  // For demo, assume 'user' role (could also be passed via query if needed)
...
app.post('/patients', async (req, res) => {
  // Accept clinic_id from frontend for now
...
app.delete('/patients/:id', async (req, res) => {
  const { id } = req.params;
  // Optionally, you can accept clinic_id from req.body for extra safety
  try {
    const result = await pool.query(
...
// --- NEW: Mount invoices, payments, and procedures routers ---
app.use('/api/invoices', invoicesRouter);
app.use('/api/payments', paymentsRouter);
app.use('/api', proceduresRouter); // <--- NEW
