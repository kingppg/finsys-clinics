...
**Still NOT logic-audited (unchanged from §p):** Appointments component internals (`AppointmentForm.js` validation/conflict/timezone-on-save, `AppointmentsCalendar.jsx`, `AppointmentsTable.jsx` calendar-math/filter/socket-reconciliation), and Patients/Dentists/Odontogram/PatientFiles/ClinicConfig/auth logic. Security sweep done there, logic audit not.
