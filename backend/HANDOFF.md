...
**Still deferred (billing) — Phases 2–3 of the VAT/discount work:** **Phase 2** = per-procedure **`sc_pwd_eligible`** flag → mixed invoices (discount/VAT only on eligible line items, e.g. cosmetic whitening charged normally on a senior's bill). **Phase 3** = immutable per-invoice audit snapshots (freeze VAT status/rate/amount + discount type/rate/amount + **Senior/PWD ID number** so old invoices don't change if the clinic flips VAT). Also: persisting an `appointment_id` FK on invoices (column existence unconfirmed); Manage modal still on `.bills-modal-overlay` (not `.dc-overlay`).
