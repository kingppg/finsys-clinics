...
- **✅ RESOLVED — Booking bot / Claire re-verified + documented (2026-07-07, see §q).** The confirmation pass confirmed all prior fixes still standing AND surfaced new standing defects (F1–F6 + a socket leak + an invalid-date cascade), which were fixed and deployed. This item no longer resurfaces — the outcome lives in §q.
...
### q) BOOKING BOT re-verification + logic fixes, and PHONE-REQUIRED / SMS-ready rollout (2026-07-07 continued) ⭐ closes the booking-bot item in §p
Third deep session. Two bodies of work, both DEPLOYED to main; migration 022 RUN in Supabase. Offline harness `backend/test-webhook.js` extended to **59 checks (all green)** — it drives the real state machine + real claire history with axios/Supabase/Anthropic stubbed; run `node test-webhook.js`.

**Booking-bot re-verification (the §p pending item).** Read `webhook.js` + `helpers/` + `ai/claire.js` in full, twice (adversarial second pass). **All previously-recorded fixes are still standing** (Claire history alternation, confidence gating 0.6, FB signature verify, the 14 Bug-# fixes, session persistence). But the confirmation pass ALSO found real standing defects — so "audited before" ≠ "clean". Fixed + deployed in `9d2698d`:
- **F1 — confirm-by-code enumeration → account/guardian takeover.** Codes are short sequential ints; the old flow linked a Messenger account / granted guardian confirm+cancel rights on possession alone. Now an unrecognized sender must pass **name verification** (new `awaiting_confirm_verify` state, 2 attempts then locked to the clinic) before any identity write. Claire prompt learned the new state.
- **F3 — self-confirm was a silent no-op** (worse than first thought). Root cause: the code re-read the patient AFTER writing the link, saw "already linked", and skipped the status update — so EVERY self-confirmation said "✅ confirmed" while the row stayed `Scheduled`. Extracted `finalizeCodeConfirmation()` which captures link state BEFORE any write; status now genuinely flips (harness asserts it).
- **F2 + invalid-date cascade — fake "booked!" confirmations.** The date gate was regex-only (`\d{4}-\d{2}-\d{2}`) so `2026-07-32` parsed to Invalid Date, sailed through NaN comparisons, poisoned `getAvailableSlots` (whose query errors were swallowed → "all slots free"), and the final insert error was ignored → user told "booked" with a blank code. Fixed: real calendar validation + the appointment insert checks its error + `getAvailableSlots`/confirm-loop now **fail CLOSED** on query errors.
- **F4 — Cancelled-row recycling.** Confirming used to overwrite an existing (Cancelled) row at that dentist+time, destroying cancellation history + the analytics rows §a records, and could hijack a concurrently-confirmed appointment under a race. Now **always inserts a fresh row**.
- **F5 — "for whom → someone else" booked the wrong person.** The unlinked-self double-booked path entered `awaiting_for_whom` with `patient_name` = the SENDER'S name; the old shortcut then skipped to slots and booked the double-booked sender under "someone else". Now resets identity + re-asks the name; double-booking is **re-checked at the moment of insert**; `MENU_BOOK_APPOINTMENT` clears stale session data.
- **F6 + minors** — past-date check uses clinic timezone (closed a 00:00–08:00 Manila window); weekday math host-TZ-independent (`getDayOfWeek`); cancel insert error checked; `sendTimeSlotButtonTemplate` wrapped in try/catch; `awaiting_my_confirm_match` got the missing `cancel_flow` escape.
- **Socket leak** — `emitAppointmentUpdate` did a bare `io.emit(fullRow)`; the Socket.IO server has **no auth/rooms**, so any client with a whitelisted Origin received every clinic's appointment rows (reason free-text, messenger/guardian ids). Now emits a **whitelisted column set** only (ids/status/times/checked_in/deleted/booking_origin — everything the frontend listeners read, nothing more). Frontend already filters clinic_id client-side. **Pattern for future socket work: never emit `select('*')`; whitelist, and consider clinic rooms + a join handshake if payloads ever need PII.**

**PHONE now REQUIRED + validated everywhere (SMS is the reminder fallback for Meta's closed 24h window).**
- **New shared validator `backend/helpers/phone.js`** — `normalizePHMobile(raw)` → canonical `09XXXXXXXXX` or null (accepts `09…/+639…/639…/9…` with spaces/dashes; rejects landlines + junk the old `/^\d{10,}$/` passed), and `formatForProvider(raw, provider)` → **Twilio E.164 (`+63…`) / Semaphore local (`09…`)**. `bookingHelpers` re-exports `normalizePHMobile`.
- **Bot:** both phone-collection steps are now REQUIRED (removed the whole skip-phrase path); invalid input re-prompts.
- **SMS senders (all three: `reminderScheduler.js` + `routes/reminders.js` + `routes/statusNotifications.js`)** now call `formatForProvider` at send time — they used to pass the stored number verbatim (fine for Semaphore, WRONG for Twilio which needs E.164). Tolerant of any stored format, so a Twilio switch needs no data change.
- **Frontend (`76c5e74`):** new **`frontend/src/utils/phone.ts`** mirrors the backend validator exactly. Wired into BOTH staff patient-create paths — **Patients.js** (Add/Edit) and **AppointmentForm.js `handleAddNew`** (quick-add inside booking; the second, easily-missed door). Phone was already required-non-empty, but junk passed the truthiness check and stored raw — now validated+normalized with a clear message + `09171234567` placeholder. **Net: every patient-create path (bot, Patients form, appt quick-add) enforces identical SMS-ready rules — no phone-less/junk patient can be created anywhere.**

**Migration 022 (`a64525e`, RUN, no-op) — `backend/db/migrations/022_normalize_patient_phones.sql`.** Normalizes EXISTING `patients.phone` to canonical form (backup table + ROLLBACK block + audit query). Live dry-run showed data was **already canonical → 0 rows changed**. Traps caught along the way: `patients.id` is **integer** not uuid (backup table type), and `CREATE TABLE IF NOT EXISTS` kept a stale uuid table from a first attempt → made it `DROP TABLE IF EXISTS` + fresh create. **The backup table had NO RLS (PII exposure while it existed) — dropped it after; if a future migration snapshots real data, drop it same-session or give it RLS.**

**Audit outcome (who can't we SMS):** ran the read-only null/invalid-phone query. ~90 rows, but owner confirmed **it's ALL dummy/test data — the app is live but NO real clinic is using it yet** (famous + biblical names, `12345678901` placeholder phones, a `"For Someone Else"` bot artifact id 142). No cleanup/collection needed; a pre-go-live test-data purge is the eventual task (app soft-deletes via `deleted=true`, reversible), owner's call.

**Still NOT logic-audited (unchanged from §p — NOTE: Appointments components NOW DONE, see §r):** ~~Appointments component internals~~ (done in §r) and Patients/Dentists/Odontogram/PatientFiles/ClinicConfig/auth logic. Security sweep done there, logic audit not.

### r) APPOINTMENTS component logic audit + CONFIGURABLE CLINIC SCHEDULE feature (2026-07-07→10) ⭐ the big one; read for the whole appointment cycle
Long session. Two arcs: (A) a genuine first-time logic audit of the Appointments components (the §p/§q gap), then (B) built configurable clinic scheduling end-to-end and hardened it over three adversarial audit rounds. **All commits DEPLOYED; migrations 023 + 024 RUN in Supabase.** Offline harness `backend/test-webhook.js` now **72 checks** (`node test-webhook.js`).

**A) Appointments component logic audit (`502d65a`, `4b40d83`):**
- **Silent write failures (F2-class), fixed everywhere:** supabase-js returns `{error}` and does NOT throw, so unchecked writes showed fake success. Fixed in `AppointmentForm` (create/update + double-booking read), `AppointmentsTable` (cancel/soft-delete, status-change — checked BEFORE the optimistic UI flip + patient SMS), and later `DentistAvailabilityManager` (all 3 write paths, in the third audit round). **RULE: always `const { error } = await supabase…; if (error) throw/handle` — it never throws on its own.**
- **Double-booking backstop — migration 023 (`prevent_double_booking.sql`, RUN):** partial UNIQUE index `uniq_active_dentist_slot` on `(dentist_id, appointment_time) WHERE deleted=false AND (status IS NULL OR status<>'Cancelled')`. The form catches the `23505` and shows "slot just taken" instead of a false success. Preflight dup-check was clean.
- **Unscoped booked-slot query:** `AppointmentForm.fetchBookedSlots` loaded ALL of a dentist's appts (PostgREST 1000-row cap → could drop the day's booked slots → double-book). Bounded to a 3-day window.
- **Weekly-view calendar bug:** `getWeeksOfMonth` stored week `end` at 00:00, so `d <= end` dropped every appointment on a week's last day (esp. the month-end day) from the weekly view. Now end-of-day (23:59:59.999). Node-sim verified every day in exactly one week.
- **#5 timezone date-filtering (`4b40d83`):** `AppointmentsTable` filtered/week-mathed in BROWSER-local while displaying clinic-local → near-midnight appts on the wrong day cross-TZ. Added `clinicYMD()` (clinic-tz calendar parts via `Intl`); routed all filters through it.
- **#7 dead code removed:** `AppointmentsCalendar.jsx` deleted (unimported; hit a hardcoded localhost + the unmounted `/appointments/all` router, no clinic filter).
- **Hardcoded localhost socket:** `AppointmentForm` used `io('http://localhost:5000')` (no live refresh in prod) → now the shared `../socket` singleton.

**B) CONFIGURABLE CLINIC SCHEDULE (the feature). Was hardcoded: closed Sunday, 09–18, 20-min slots, lunch 12–1. Now per-clinic: per-weekday open/close + closed, multiple breaks, slot interval (15/20/25/30/35/40/45/60), and holidays (recurring + one-off, each blockable). Applied app-wide (bot + form + availability). Theme picker is PER-USER (localStorage), not clinic-wide.**
- **Migration 024 (`clinic_schedule.sql`, RUN):** additive — `clinics.schedule` JSONB (default = today's exact behavior, backfilled) + column grants (`clinics` is column-grant secured, not RLS) + new RLS'd per-clinic `clinic_holidays` table. `schedule` shape: `{ days:{"0".."6":{is_closed,open,close}}, breaks:[{start,end,label}], slot_interval_minutes }` (day keys = `getDay()`, Sun=0).
- **ONE shared engine, mirrored (like phone.js/phone.ts):** `backend/helpers/schedule.js` ⇄ `frontend/src/utils/schedule.ts` — `normalizeSchedule`, `dowOf`, `daySlots` (grid+isBreak, whole slots inside open→close, break = overlap), `bookableSlots`, `closedReasonFor` ('day'|'holiday'|null), `blockedHolidayFor`, `describeSchedule`. **Keep the two byte-identical.** Verified default output == old behavior.
- **Phase 1 (`baad272`):** storage + a "Clinic Schedule" rail in Clinic Config (`frontend/src/components/clinic/ClinicSchedule.tsx`, self-contained — loads/saves ONLY its own data, never ClinicConfig's `formData`/save path). Holidays save immediately (list); hours/breaks/interval via "Save Schedule".
- **Phase 2 bot (`97472a0`):** `getAvailableSlots` reads schedule + `clinic_holidays`, returns `closedReason`; Claire's prompt hours generated from `describeSchedule`; **RANGE-MATCH booked detection** + `isDentistSlotFree` helper for the confirm path. Per-dentist preload (not per-slot).
- **Phase 3 form (`df36f2c`) + Step 4 availability (`52c8de9`):** both read the same engine. **AvailabilityManager was rebuilt from a fixed hourly 9–17 grid to the day's REAL slots (`daySlots` at the configured interval)** — critical because non-divisor intervals (25/35/40/45) don't align to hours. Editing on the slot grid; storage still merges contiguous blocked slots into aligned ranges. Playwright-verified live (45-min grid renders, Sunday-open selectable, data reverted).

**Three audit rounds on the schedule integration found + fixed (`29ea0aa`, `f7a22ec`):**
- **F1 (real bug) — block enforcement was "slot-start-in-block", must be OVERLAP.** A slot straddling a block's start (e.g. a 45-min 15:45 slot into a 16:00 block, or any legacy hour-aligned block on a non-divisor grid) was OFFERED → dentist booked into blocked time. Fixed at all 3 sites (bot `getAvailableSlots`+`isDentistSlotFree`, form `fetchBlockedSlots`) to `slotStart < blockEnd && slotStart + interval > blockStart` — now matches availability + breaks. Harness +3 straddle checks. (I seeded this asymmetry in Phase 2: range-matched BUSY but left BLOCK start-in. BUSY stays containment — appts have no stored duration.)
- **F2 — clinic-TZ booked-mapping:** form + availability now map appts by clinic-local date/time (`Intl`, matching the bot), not browser-local. Identical on-site, correct cross-TZ.
- **F3 — recurring `day_of_week` blocks:** the FORM now honors both `specific_date` and `day_of_week` (matches the bot). The availability editor still can't author/show recurring blocks — that's a **feature, not a fix** (per-date editor model); NO `day_of_week` blocks exist today (no UI creates them), so fully latent. See the F3 explanation in memory.
- **Holiday tooltips:** hovering a closed date explains why ("Closed — <Holiday> (holiday)"). Availability calendar switched `disabled`→`aria-disabled` because **`title` doesn't fire on `disabled` buttons in Chrome**; form (react-calendar) uses a visible dot + tint + legend.

**Honest posture:** the whole appointment cycle (bot/form/availability/engine) is consistent — overlap block enforcement, no fake-success in any write path, clinic-TZ mapping, DB double-book backstop, holidays explained. **Only genuinely open = the recurring-availability FEATURE (F3), owner's call. Next modules (unaudited logic): Patients/Dentists/Odontogram/PatientFiles/ClinicConfig/auth.**
...
| `9d2698d` | **fix(webhook): booking-bot logic hardening — confirm-code name-verify (F1), self-confirm actually confirms (F3), calendar validation + honest insert errors (F2), fresh-row insert not Cancelled-recycle (F4), for-whom books right person (F5), clinic-TZ past-date (F6), socket payload whitelist; phone REQUIRED + `helpers/phone.js`; SMS provider formatting in all 3 senders; harness → 59 checks** |
| `a64525e` | **fix(migration-022): normalize existing patients.phone (RUN, no-op — data already canonical); DROP+CREATE bigint backup table (id is int; stale-uuid trap)** |
| `76c5e74` | **feat(patients): require + validate PH mobile on BOTH staff create paths (Patients Add/Edit + AppointmentForm quick-add) via shared `frontend/src/utils/phone.ts` mirroring the backend** |
| `502d65a` | **§r-A appointments logic audit — silent write failures fixed; migration 023 double-book unique index (RUN); 3-day booked-slot window; weekly-view end-of-day fix** |
| `4b40d83` | **§r-A clinicYMD clinic-tz date filtering (#5); deleted dead AppointmentsCalendar.jsx (#7)** |
| `baad272` | **§r-B configurable schedule Phase 1 — clinics.schedule + clinic_holidays (migration 024 RUN) + Clinic Schedule rail (ClinicSchedule.tsx) + per-USER theme picker** |
| `97472a0` | **§r-B Phase 2 bot — schedule.js engine; getAvailableSlots schedule-driven + range-match + holidays + closedReason; Claire prompt from schedule; isDentistSlotFree** |
| `df36f2c` | **§r-B Phase 3 form — utils/schedule.ts mirror; AppointmentForm reads schedule (grid/breaks/closed/holidays) + range-match** |
| `52c8de9` | **§r-B Step 4 availability — DentistAvailabilityManager grid = daySlots (any interval); AvCalendar disables closed days+holidays; slot-aligned block ranges** |
| `29ea0aa` | **§r sweep — F1 OVERLAP block enforcement (bot×2 + form); F2 clinic-TZ booked-mapping; F3 form honors day_of_week blocks** |
| `f7a22ec` | **§r round-3 — fake-success killed in all 3 availability writes; holiday tooltips (aria-disabled for Chrome); blockedHolidayFor in both mirrors; clinic-tz default date** |
